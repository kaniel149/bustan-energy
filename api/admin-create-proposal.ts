...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const {
      ...
      battery_kwh = 10,
      // Financials are computed server-side via calculateSolarFinancials (P0:
      // no fallback to client-submitted annual_kwh/savings/payback — fail loud).
      total_price_thb,
      roof_original_url,
      roof_panels_url,
    ...
    // Hard-fail if the authoritative engine produced implausible output.
    // A zero annual_kwh means system_size_kwp resolved to 0 — the proposal
    // numbers would be nonsensical.  Never silently fall back to stale
    // client-submitted figures: that hides the root cause and sends wrong
    // financials to the client PDF.
    if (!financials.annual_kwh || !Number.isFinite(financials.annual_kwh)) {
      return Response.json(
        { ok: false, error: 'financial_calc_failed', detail: 'calculateSolarFinancials returned zero annual_kwh — check system_size_kwp and panel_count inputs' },
        { status: 422 },
      )
    }
    ...
    const payback_with_tax = financials.payback_with_tax_years || 0
    ...
    const upsertBody: Record<string, unknown> = {
      ...
      metadata: {
        ...
        // On-roof visual URLs (generated by admin-overlay-panels, persisted so
        // edit-mode can restore them without re-triggering the expensive Gemini call).
        roof_original_url: roof_original_url || null,
        roof_panels_url: roof_panels_url || null,
