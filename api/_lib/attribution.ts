...
//
// Ported from Solaris Panama platform/api/lib/attribution.ts.
//
...
//   4. ttclid present → utm_source='tiktok', utm_medium='paid_social', utm_campaign='ttclid_inferred'
//   5. referrer_source provided (from client storage) → use referrer fallback
...
export interface RawAttribution {
  ...
  fbclid?: string | null
  ttclid?: string | null
  /** Referrer-inferred source passed from the client ('google_organic' | 'facebook' | 'instagram' | 'bing_organic' | 'yahoo_organic' | 'tiktok') */
  referrer_source?: string | null
  /** Raw document.referrer captured on the client */
  referrer_url?: string | null
}

export interface AttributionDebug {
...
export interface ResolvedAttribution {
  ...
  fbclid: string | null
  ttclid: string | null
  /** Debug payload written to projects.attribution_debug and webhook_logs */
...
export function inferAttribution(raw: RawAttribution): ResolvedAttribution {
  ...
  // Rule 4: ttclid present → TikTok paid click
  else if (raw.ttclid) {
    utm_source = 'tiktok'
    utm_medium = 'paid_social'
    utm_campaign = raw.utm_campaign || 'ttclid_inferred'
    debug.inferred = true
    debug.infer_reason = 'ttclid_present'
  }
  // Rule 5: referrer_source from client storage
