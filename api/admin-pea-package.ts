...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // ── Call internal drawing API via fetch (avoids code duplication) ──
    // We call admin-pea-drawings with a service-auth header trick or
    // generate the four drawings inline. We use inline generation here
    // for SLD + Specs; Electrical Plan + Layout Plan use dedicated generators.
    ...
    for (const d of drawingConfigs) {
      ...
      const row = await supaPost('pea_documents', {
        ...
        metadata: { storage_path: storagePath, generator: 'admin-pea-package' },
      }).catch(() => null)
