...
function ResortAssessmentForm() {
  ...
  return (
    <div className="rounded-[2rem] border border-grove/15 bg-white/90 p-6 shadow-[0_24px_80px_rgba(36,70,62,0.12)] md:p-8">
      ...
      <div className="grid gap-3">
        <input className="rounded-2xl border border-grove/15 bg-white px-4 py-3 outline-none transition focus:border-ocean" placeholder="Full name" value={form.name} onChange={(e) => update('name', e.target.value)} />
        <input className="rounded-2xl border border-grove/15 bg-white px-4 py-3 outline-none transition focus:border-ocean" placeholder="Business / property name" value={form.property} onChange={(e) => update('property', e.target.value)} />
        <div className="grid gap-3 md:grid-cols-2">
          <input className="rounded-2xl border border-grove/15 bg-white px-4 py-3 outline-none transition focus:border-ocean" placeholder="WhatsApp / phone" value={form.contact} onChange={(e) => update('contact', e.target.value)} />
          <input className="rounded-2xl border border-grove/15 bg-white px-4 py-3 outline-none transition focus:border-ocean" placeholder="Email" value={form.email} onChange={(e) => update('email', e.target.value)} />
        </div>
        <input className="rounded-2xl border border-grove/15 bg-white px-4 py-3 outline-none transition focus:border-ocean" placeholder="Property location" value={form.location} onChange={(e) => update('location', e.target.value)} />
        <div className="grid gap-3 md:grid-cols-2">
          <select className="rounded-2xl border border-grove/15 bg-white px-4 py-3 outline-none transition focus:border-ocean" value={form.propertyType} onChange={(e) => update('propertyType', e.target.value)}>
          ...
          <select className="rounded-2xl border border-grove/15 bg-white px-4 py-3 outline-none transition focus:border-ocean" value={form.billRange} onChange={(e) => update('billRange', e.target.value)}>
        ...
        <textarea className="min-h-28 rounded-2xl border border-grove/15 bg-white px-4 py-3 outline-none transition focus:border-ocean" placeholder="What are you trying to solve? High bill, outages, battery decision, quote review, new build, ESG, O&M..." value={form.concern} onChange={(e) => update('concern', e.target.value)} />
        <a
          href={whatsappUrl}
          target="_blank"
    ...
    <main className="bustan-home overflow-hidden bg-[var(--bustan-paper)] text-ink">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.08fr_.92fr]">
          <div>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-gold/40 bg-gold/10 px-4 py-2 text-sm font-semibold text-gold">
              <Waves size={16} strokeWidth={1.5} aria-hidden />
            ...
            <div className="mt-9 flex flex-col gap-4 sm:flex-row">
              <a href="#assessment" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-gold px-7 py-4 font-semibold text-grove shadow-xl shadow-gold/20 transition hover:-translate-y-0.5">
                Get a Solar Savings Check
                <ArrowRight size={18} strokeWidth={1.5} aria-hidden />
              </a>
              <a href={checklistUrl} download className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-shell/30 bg-shell/10 px-7 py-4 font-semibold text-shell transition hover:bg-shell/16">
            ...
            <div className="mt-9 grid max-w-3xl gap-3 sm:grid-cols-2">
              {['Assessment-first: no generic panel sizing', 'Solar, battery, backup, monitoring and O&M', 'Built for tropical coastal hospitality', 'Local-execution friendly planning'].map((item) => (
                <div key={item} className="rounded-2xl border border-shell/15 bg-shell/8 px-4 py-3 text-sm font-medium text-shell/82">
                  {item}
                </div>
              ))}
            </div>
          ...
          <div className="rounded-[2rem] border border-shell/15 bg-shell/10 p-5 shadow-2xl shadow-black/20 backdrop-blur">
            ...
            <div className="mt-5 rounded-[1.5rem] bg-shell p-6 text-ink">
              <div className="text-xs font-bold uppercase tracking-[0.18em] text-ocean">What we clarify</div>
              <div className="mt-2 font-serif text-4xl font-semibold tracking-tight text-grove">Solar fit</div>
              <p className="mt-3 text-sm leading-6 text-ink/70">Bill impact, battery value, payback assumptions, project risks and the right next step.</p>
              <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
                {['Bill review', 'Battery check', 'Quote review', 'O&M lens'].map((item) => (
                  <div key={item} className="rounded-2xl bg-[var(--bustan-paper)] px-4 py-3 font-semibold text-grove">{item}</div>
                ))}
              </div>
            </div>
          </div>
      ...
      <section className="px-6 py-20">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.9fr_1.1fr]">
          <div>
            <div className="text-sm font-bold uppercase tracking-[0.18em] text-ocean">The real problem</div>
          ...
          <div className="rounded-[2rem] border border-grove/12 bg-white/80 p-8 shadow-lg shadow-grove/5">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-2">
              {['Seasonal usage spikes', 'Outages or voltage issues', 'Unclear battery payback', 'Quote risk before analysis'].map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-2xl bg-[var(--bustan-paper)] px-4 py-3 text-sm font-semibold text-grove">
                  <CheckCircle2 size={18} strokeWidth={1.5} className="text-ocean" aria-hidden />
                  {item}
                </div>
              ))}
            </div>
      ...
      <section className="bg-shell px-6 py-20">
        <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2">
          <div className="rounded-[2rem] bg-grove p-8 text-shell md:p-10">
            <div className="text-sm font-bold uppercase tracking-[0.18em] text-gold">Our philosophy</div>
            <h2 className="mt-4 font-serif text-4xl leading-tight tracking-tight md:text-6xl">Assess first. Install second.</h2>
            <p className="mt-5 text-lg leading-8 text-shell/82">The most expensive mistake is starting with equipment before understanding the business case.</p>
          </div>
          <div className="rounded-[2rem] border border-grove/12 bg-white p-8 md:p-10">
            <h3 className="text-2xl font-semibold tracking-tight text-grove">Bustan helps hospitality owners make smarter solar decisions.</h3>
            <p className="mt-4 leading-7 text-ink/70">We start with your bill, property, operating loads, business goals and local constraints — not with a panel count. Bustan supports transparent planning, local partner enablement, practical O&M and monitoring.</p>
          </div>
        </div>
      </section>
      ...
      <section id="assessment" className="px-6 py-20">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.95fr_1.05fr]">
          <div>
            ...
            <p className="mt-5 text-lg leading-8 text-ink/70">A focused review that gives you a clearer view of solar fit, battery fit, savings potential, risk areas and recommended next steps.</p>
            <div className="mt-8 grid gap-3">
              {assessmentIncludes.map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-2xl bg-white/80 px-4 py-3 text-sm font-semibold text-grove shadow-sm shadow-grove/5">
                  <CheckCircle2 size={18} strokeWidth={1.5} className="shrink-0 text-ocean" aria-hidden />
                  {item}
                </div>
              ))}
            </div>
      ...
      <section className="bg-shell px-6 py-20">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <div className="text-sm font-bold uppercase tracking-[0.18em] text-ocean">Simple path</div>
            <h2 className="mt-4 font-serif text-4xl leading-tight tracking-tight text-grove md:text-6xl">How it works</h2>
          </div>
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {[
              ...
              <div key={num} className="rounded-[2rem] border border-grove/12 bg-white p-7">
                <div className="mb-6 grid h-12 w-12 place-items-center rounded-2xl bg-[var(--bustan-paper)] text-lg font-bold text-grove">{num}</div>
      ...
      <section className="px-6 py-20">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <div className="text-sm font-bold uppercase tracking-[0.18em] text-ocean">Benefits</div>
            <h2 className="mt-4 font-serif text-4xl leading-tight tracking-tight text-grove md:text-6xl">Better decisions before bigger commitments</h2>
          </div>
          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {benefits.map(({ title, body, icon: Icon }) => (
              <div key={title} className="rounded-[2rem] border border-grove/12 bg-white/85 p-6 shadow-sm shadow-grove/5">
                <div className="mb-5 inline-grid h-12 w-12 place-items-center rounded-2xl bg-ocean/10 text-ocean"><Icon size={22} strokeWidth={1.5} aria-hidden /></div>
      ...
      <section className="px-6 py-20">
        <div className="mx-auto grid max-w-7xl items-center gap-8 rounded-[2.5rem] bg-grove p-8 text-shell md:grid-cols-[1.1fr_.9fr] md:p-12">
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-gold/12 px-4 py-2 text-sm font-bold uppercase tracking-[0.14em] text-gold"><FileText size={16} strokeWidth={1.5} aria-hidden /> Free checklist</div>
            <h2 className="font-serif text-4xl leading-tight tracking-tight md:text-6xl">7 Solar + Battery Mistakes Resorts in Thailand Should Avoid</h2>
            <p className="mt-5 text-lg leading-8 text-shell/82">Learn why a quote should not be the first step, when batteries can become an expensive mistake, and which hospitality loads change the ROI calculation.</p>
          </div>
          <a href={checklistUrl} download className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-gold px-7 py-4 font-semibold text-grove shadow-xl shadow-black/10 transition hover:-translate-y-0.5">
      ...
      <section className="px-6 py-20">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.85fr_1.15fr]">
          <div>
            ...
            <h2 className="mt-4 font-serif text-4xl leading-tight tracking-tight text-grove md:text-6xl">Questions buyers ask before moving forward</h2>
          </div>
          <div className="grid gap-3">
            {faqItems.map((item, index) => (
              <details key={item.q} className="group rounded-2xl border border-grove/12 bg-white/90 p-5" open={index === 0}>
                <summary className="cursor-pointer list-none text-lg font-semibold text-grove">{item.q}</summary>
      ...
      <section className="px-6 pb-24 pt-8">
        <div className="mx-auto grid max-w-7xl items-center gap-8 rounded-[2.5rem] bg-[linear-gradient(135deg,var(--bustan-grove),#132d27)] p-8 text-shell md:grid-cols-[1.2fr_.8fr] md:p-12">
          <div>
            <h2 className="font-serif text-4xl leading-tight tracking-tight md:text-6xl">Want to know if solar + battery is right for your property?</h2>
            <p className="mt-5 text-lg leading-8 text-shell/82">Send your bill and property details. Bustan Energy will help you understand the opportunity, risks and next step before you invest.</p>
          </div>
          <a href="#assessment" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-gold px-7 py-4 font-semibold text-grove shadow-xl shadow-black/10 transition hover:-translate-y-0.5">
            Get a Solar Savings Check
            <Zap size={18} strokeWidth={1.5} aria-hidden />
          </a>
        </div>
      </section>
    </main>
