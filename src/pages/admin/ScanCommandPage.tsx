...
import { viewportScanArea, scanJobSummary } from '../../lib/scan-assessment'
    ...
    <div className="relative flex min-h-0 flex-1">
      ...
      {jobsOpen && <section dir="rtl" aria-label="סריקת אזור והיסטוריה" className="absolute inset-y-0 left-0 z-20 w-full max-w-md overflow-y-auto border-r border-[#24463E]/20 bg-[#FFF4E2] p-5 shadow-xl">
        ...
        <div className="mt-3 space-y-3">{jobs.map((job) => {
          const counts = scanJobSummary(job.counts)
          return <article key={job.id} className="rounded-xl border border-[#24463E]/15 bg-white/60 p-3"><div className="flex items-center justify-between gap-2"><span className={`text-xs font-semibold ${job.status === 'failed' ? 'text-red-700' : 'text-[#24463E]'}`}>{STATUS_LABEL[job.status]}</span><time className="text-[10px] text-[#27342F]/55">{new Date(job.created_at).toLocaleString('he-IL', { dateStyle: 'short', timeStyle: 'short' })}</time></div>{job.status === 'done' && <p className="mt-2 text-xs">{counts.added?.toLocaleString('he-IL') ?? 'לא דווח כמה'} גגות נוספו · {counts.examined?.toLocaleString('he-IL') ?? 'לא דווח כמה'} רשומות נבדקו</p>}{counts.coverage === 'partial' && <p className="mt-2 text-xs leading-relaxed text-amber-800">כיסוי חלקי: מקור חסר, מתאר לא נתמך או מגבלת עיבוד. סרקו אזור קטן יותר והשלימו בדיקה במפה.</p>}{job.status === 'done' && counts.coverage === 'processed' && <p className="mt-2 text-[11px] text-[#27342F]/60">המקורות הזמינים עובדו. נדרשת בדיקת כיסוי מול התצלום.</p>}{job.status === 'done' && counts.coverage === 'unknown' && <p className="mt-2 text-[11px] text-[#27342F]/60">סריקה היסטורית ללא מדד כיסוי. יש לבדוק את הגגות מול התצלום.</p>}{job.status === 'failed' && <p className="mt-2 text-xs text-red-700">לא התקבלו תוצאות מלאות. נסו אזור קטן יותר או סריקה חדשה.</p>}{job.bbox?.length === 4 && <button className="mt-2 text-xs font-medium text-[#24463E] underline underline-offset-4" onClick={() => { const b = job.bbox!; mapRef.current?.fitBounds([[b[0], b[1]], [b[2], b[3]]], { padding: 30 }); setJobsOpen(false); setMobileView('map') }}>הצגת אזור הסריקה</button>}</article>
        })}</div>
      </section>}
    </div>
  </div>
