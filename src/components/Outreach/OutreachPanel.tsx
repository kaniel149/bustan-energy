...
// Canonical outreach approval queue. Mounted in two places:
//   • platform main nav   → src/pages/PlatformPage.tsx (platformView === 'outreach')
//   • admin sidebar route → src/pages/admin/OutreachPage.tsx (re-export)
    ...
    <div dir="rtl" className="p-3 sm:p-6 max-w-[1200px] mx-auto pb-24 sm:pb-6 space-y-4">
      <div className="flex items-baseline justify-between gap-3 flex-wrap">
        ...
        <p className="text-white/40 text-xs">
          הודעות ראשונות מותאמות אישית — אישור לפני שליחה
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white/5 rounded-2xl border border-white/10 p-12 text-center space-y-1">
          <p className="text-white/50 text-sm">אין הודעות בסטטוס זה.</p>
          {tab === 'draft' && (
            <p className="text-white/30 text-xs">
              טיוטות נוצרות אוטומטית מגגים סרוקים עם איש קשר ואימייל (cron יומי 02:00).
            </p>
          )}
        </div>
      ) : (
